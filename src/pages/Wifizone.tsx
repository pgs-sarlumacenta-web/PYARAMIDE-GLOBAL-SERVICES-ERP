...
import { PlusIcon, PencilIcon, TrashIcon, TicketIcon, ChartPieIcon, WifiIcon, ClipboardDocumentIcon, Cog6ToothIcon, BanknotesIcon, ScaleIcon, ArrowTrendingUpIcon, UserGroupIcon, PrinterIcon, RectangleStackIcon } from '@heroicons/react/24/outline';
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, PieChart, Pie, Cell, Legend } from 'recharts';
import WifizoneRecu from '../components/wifizone/WifizoneRecu.tsx';
import WifizoneBulkTicketSheet from '../components/wifizone/WifizoneBulkTicketSheet.tsx';
...
const Wifizone: React.FC = () => {
    ...
    const [bulkResult, setBulkResult] = useState<{ sales: WifizoneSale[], plan: WifizonePlan, client?: Client } | null>(null);
    ...
    const [bulkSaleData, setBulkSaleData] = useState({ planId: '', clientId: '', quantity: 20 });
    ...
    const openBulkModal = useCallback(() => {
        ...
        setBulkSaleData({ planId: firstPlan.id, clientId: '', quantity: 20 });
    ...
    const handleBulkSubmit = () => {
        const plan = wifizonePlans.find(p => p.id === bulkSaleData.planId);
        if (!plan) return;

        const quantity = Math.max(1, Math.min(200, bulkSaleData.quantity)); // Limit to 200 at a time
        const newSales: WifizoneSale[] = [];
        let currentReceiptNum = billingSettings.receiptNextNumber;

        // Generate batch ID
        const batchId = `BATCH-${Date.now()}`;

        for (let i = 0; i < quantity; i++) {
             const newId = `WZS-${Date.now()}-${i}`;
             const newReceiptRef = `${billingSettings.receiptPrefix}WIFI-${currentReceiptNum}`;
             currentReceiptNum++;

             newSales.push({
                id: newId,
                planId: plan.id,
                clientId: bulkSaleData.clientId || undefined,
                saleDate: new Date().toISOString(),
                voucherCode: generateVoucherCode(),
                totalAmount: plan.price,
                receiptRef: newReceiptRef,
        ...
        setWifizoneSales(prev => [...newSales, ...prev]);
        setBillingSettings(prev => ({ ...prev, receiptNextNumber: currentReceiptNum }));

        // Create ONE transaction for the whole bulk to avoid clutter
        const totalAmount = plan.price * quantity;
        const client = clients.find(c => c.id === bulkSaleData.clientId);
        const newTransaction: Transaction = {
            id: `T_WIFI_${batchId}`,
            date: new Date().toISOString(),
            department: 'Wifizone',
            description: `Génération Lot (${quantity} tickets) - Forfait: ${plan.name} ${client ? `- Agent: ${client.name}` : ''}`,
            amount: totalAmount,
            type: 'Revenu',
        };
        setTransactions(prev => [newTransaction, ...prev]);

        addLogEntry('Vente Groupée Wifizone', `A généré ${quantity} tickets ${plan.name} pour un total de ${totalAmount.toLocaleString()} GNF`, 'wifizoneSale');
        setIsBulkModalOpen(false);
        setBulkResult({ sales: newSales, plan, client });
    };
    ...
    const downloadBulkSheet = () => {
        ...
        const { sales, plan, client } = bulkResult;
        const docTitle = `Planche_WIFI_${plan.name}_x${sales.length}`;
        const docContent = <WifizoneBulkTicketSheet sales={sales} plan={plan} companyProfile={companyProfile} agentName={client?.name} />;
        setDocumentToDownload({ content: docContent, format: 'pdf', title: docTitle });
        showAlert("Téléchargement", "La planche de tickets va être téléchargée.");
    };
        ...
        <div className="space-y-6">
                ...
                {activeTab === 'ventes' && (
                    <div>
                        <div className="flex justify-end mb-4 space-x-2">
                            {hasPermission(Permission.MANAGE_WIFIZONE) && (
                                <button onClick={openBulkModal} className="btn-secondary flex items-center text-wifizone-orange border-wifizone-orange hover:bg-orange-50 dark:hover:bg-orange-900/20 dark:border-orange-700 dark:text-orange-400">
                                    <RectangleStackIcon className="h-5 w-5 mr-2" />
                                    Générer des Lots (Agents)
                                </button>
                            )}
                            <button onClick={openSaleModal} className="btn-primary bg-wifizone-orange flex items-center">
                                <PlusIcon className="h-5 w-5 mr-2" />
                                Nouvelle Vente
                            </button>
                        </div>
                        <table className="w-full text-left"><thead><tr><th>Date</th><th>Forfait</th><th>Client/Agent</th><th>Code Voucher</th><th>Montant</th></tr></thead><tbody>{wifizoneSales.slice().reverse().map(s => { const p = wifizonePlans.find(pl => pl.id === s.planId); const c = clients.find(cl => cl.id === s.clientId); return <tr key={s.id} className="border-b"><td>{new Date(s.saleDate).toLocaleString()}</td><td>{p?.name}</td><td>{c?.name || 'Au comptoir'}</td><td className="font-mono font-bold">{s.voucherCode}</td><td>{s.totalAmount.toLocaleString()} GNF</td></tr>})}</tbody></table>
                    </div>
                )}
            ...
            <Modal isOpen={isSaleModalOpen} onClose={() => setIsSaleModalOpen(false)} title="Nouvelle Vente Wifizone">
                <form onSubmit={e => {e.preventDefault(); handleSaleSubmit()}} className="space-y-4">
                    <div><label className="block text-sm font-medium mb-1">Forfait</label><select value={newSaleData.planId} onChange={e=>setNewSaleData(p=>({...p, planId: e.target.value}))} className="input-style" required>{wifizonePlans.filter(p=>!p.isArchived).map(p=><option key={p.id} value={p.id}>{p.name} - {p.price.toLocaleString()} GNF</option>)}</select></div>
                    <div><label className="block text-sm font-medium mb-1">Client (facultatif)</label><div className="flex items-center"><select value={newSaleData.clientId} onChange={e=>setNewSaleData(p=>({...p, clientId: e.target.value}))} className="input-style flex-grow"><option value="">-- Vente au comptoir --</option>{clients.filter(c=>!c.isArchived).map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select><button type="button" onClick={() => setIsAddClientModalOpen(true)} className="ml-2 btn-secondary p-2 text-sm">+</button></div></div>
            ...
            {/* Modal Vente Groupée (Bulk) */}
            <Modal isOpen={isBulkModalOpen} onClose={() => setIsBulkModalOpen(false)} title="Génération de Lots (Agents)">
                 <form onSubmit={e => {e.preventDefault(); handleBulkSubmit()}} className="space-y-4">
                    <div className="bg-blue-50 dark:bg-blue-900/30 p-3 rounded text-sm text-blue-800 dark:text-blue-200 mb-4 border-l-4 border-blue-500">
                        Cette fonction génère plusieurs tickets d'un coup pour les remettre à un agent commercial. Une seule transaction sera enregistrée.
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                        <div><label className="block text-sm font-medium mb-1">Forfait</label><select value={bulkSaleData.planId} onChange={e=>setBulkSaleData(p=>({...p, planId: e.target.value}))} className="input-style" required>{wifizonePlans.filter(p=>!p.isArchived).map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></div>
                        <div><label className="block text-sm font-medium mb-1">Quantité (Max 200)</label><input type="number" min="1" max="200" value={bulkSaleData.quantity} onChange={e=>setBulkSaleData(p=>({...p, quantity: +e.target.value}))} className="input-style" required/></div>
                    </div>
                    <div><label className="block text-sm font-medium mb-1">Agent (Client)</label><div className="flex items-center"><select value={bulkSaleData.clientId} onChange={e=>setBulkSaleData(p=>({...p, clientId: e.target.value}))} className="input-style flex-grow"><option value="">-- Stock (Pas d'agent) --</option>{clients.filter(c=>!c.isArchived).map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select><button type="button" onClick={() => setIsAddClientModalOpen(true)} className="ml-2 btn-secondary p-2 text-sm">+</button></div></div>
                    
                    <div className="mt-4 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                        <div className="flex justify-between mb-2">
                             <span>Prix unitaire :</span>
                             <span>{wifizonePlans.find(p=>p.id === bulkSaleData.planId)?.price.toLocaleString()} GNF</span>
                        </div>
                        <div className="flex justify-between font-bold text-lg border-t border-gray-300 pt-2">
                             <span>Total à payer :</span>
                             <span className="text-wifizone-orange">{( (wifizonePlans.find(p=>p.id === bulkSaleData.planId)?.price || 0) * bulkSaleData.quantity ).toLocaleString()} GNF</span>
                        </div>
                    </div>

                    <div className="flex justify-end"><button type="submit" className="btn-primary bg-wifizone-orange">Générer le lot</button></div>
                </form>
            </Modal>
                ...
                <form onSubmit={e => {e.preventDefault(); handlePlanSubmit()}} className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" placeholder="Durée" value={newPlanData.duration} onChange={e=>setNewPlanData({...newPlanData, duration: +e.target.value})} className="input-style" required/>
                        </div>
                        <div>
                            <label className="block text-sm font-medium mb-1">Prix (GNF)</label>
                            <input type="number" placeholder="Prix" value={newPlanData.price} onChange={e=>setNewPlanData({...newPlanData, price: +e.target.value})} className="input-style" required/>
                        </div>
                    </div>
            ...
            {/* Résultat Vente Groupée */}
            <Modal isOpen={!!bulkResult} onClose={() => setBulkResult(null)} title="Génération Terminée !">
                {bulkResult && (
                    <div className="text-center space-y-4">
                        <div className="flex justify-center text-green-500 mb-2">
                            <RectangleStackIcon className="h-16 w-16" />
                        </div>
                        <h3 className="text-xl font-bold">Succès !</h3>
                        <p><strong>{bulkResult.sales.length}</strong> tickets ont été générés pour le forfait <strong>{bulkResult.plan.name}</strong>.</p>
                        <p className="text-sm text-gray-500">Agent assigné : <strong>{bulkResult.client?.name || 'Aucun (Stock)'}</strong></p>
                        
                        <div className="flex justify-center space-x-4 mt-6">
                            <button onClick={downloadBulkSheet} className="btn-primary bg-wifizone-orange flex items-center px-6 py-3">
                                <PrinterIcon className="h-6 w-6 mr-2" />
                                Imprimer la planche A4
                            </button>
                        </div>
                        <button onClick={() => setBulkResult(null)} className="text-gray-500 hover:underline mt-4 text-sm">Fermer</button>
                    </div>
                )}
            </Modal>

            <AddClientModal isOpen={isAddClientModalOpen} onClose={() => setIsAddClientModalOpen(false)} onClientAdded={(client) => { setClients(prev => [client, ...prev]); setNewSaleData(p => ({...p, clientId: client.id})); setBulkSaleData(p => ({...p, clientId: client.id})); }} />
