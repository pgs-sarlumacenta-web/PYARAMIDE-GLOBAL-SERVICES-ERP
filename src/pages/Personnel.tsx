...
import { PencilIcon, TrashIcon, DocumentTextIcon, ChartPieIcon, ArrowLeftIcon, UserCircleIcon, BanknotesIcon, ArrowDownTrayIcon } from '@heroicons/react/24/outline';
            ...
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={isEditing ? 'Modifier Employé' : 'Nouvel Employé'}>
                <form onSubmit={e => {e.preventDefault(); handleFormSubmit()}} className="space-y-4">
                    ...
                    <div>
                        ...
                        <select value={newEmployeeData.department} onChange={e=>setNewEmployeeData({...newEmployeeData, department: e.target.value as any})} className="input-style">
                             ...
                             <option value="Wifizone">Wifizone</option>
                             <option value="Sécurité">Sécurité</option>
                        </select>
                    </div>
                    <div>
                        <label className="block text-sm font-medium mb-1">Salaire (GNF)</label>
             ...
             <Modal isOpen={isPayrollModalOpen} onClose={() => setIsPayrollModalOpen(false)} title={`Générer la paie pour ${currentItem?.name}`}>
                <form onSubmit={e => {e.preventDefault(); handleGeneratePayroll()}} className="space-y-4">
                    <div>
                        ...
                        <input type="text" placeholder="ex: Octobre 2023" onChange={e=>setPayrollData({...payrollData, period: e.target.value} as any)} className="input-style" required/>
                    </div>
                    <div>
                        <label className="block text-sm font-medium mb-1">Déductions (GNF)</label>
                        <input type="number" placeholder="0" onChange={e=>setPayrollData({...payrollData, deductions: +e.target.value} as any)} className="input-style" required/>
                    </div>
