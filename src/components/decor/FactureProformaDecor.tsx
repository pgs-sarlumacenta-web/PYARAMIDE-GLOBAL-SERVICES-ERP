...
interface FactureProformaDecorProps {
  order: DecorOrder;
  client: Client;
...
const FactureProformaDecor: React.FC<FactureProformaDecorProps> = ({ order, client, services, companyProfile, billingSettings }) => {
    ...
    <div className="bg-white text-black font-sans w-full max-w-4xl mx-auto p-8 border border-gray-300 shadow-lg">
      <header className="flex justify-between items-start pb-6 border-b border-gray-300">
        ...
        <div className="text-right">
            ...
            <p className="text-gray-500">Référence: {order.proformaRef}</p>
        ...
        <div className="w-full max-w-xs text-right">
          {order.discount && order.discount > 0 && (
             <div className="flex justify-between py-1">
                ...
                <span>{(order.totalAmount + order.discount).toLocaleString()} GNF</span>
            </div>
          )}
           {order.discount && order.discount > 0 && (
...
export default FactureProformaDecor;