...
export const Academie: React.FC = () => {
                ...
                <div className="mt-6">
                    ...
                    {activeFormateurDetailTab === 'paiements' && (
                        <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow">
                            ...
                            <table className="w-full text-left text-sm"><thead><tr><th className="p-2">Date</th><th className="p-2">Description</th><th className="p-2 text-right">Montant</th><th className="p-2">Réf.</th></tr></thead>
    ...
    return (
        <div className="space-y-6">
            ...
            <Modal isOpen={isScheduleModalOpen} onClose={() => setIsScheduleModalOpen(false)} title={isEditing ? 'Modifier Cours' : 'Nouveau Cours'}><form onSubmit={e => {e.preventDefault(); handleFormSubmit('schedule', newScheduleData)}} className="space-y-4"><input type="text" placeholder="Titre du cours" value={newScheduleData.title} onChange={e=>setNewScheduleData({...newScheduleData, title: e.target.value})} className="input-style" required/><select value={newScheduleData.formationId} onChange={e=>setNewScheduleData({...newScheduleData, formationId: e.target.value})} className="input-style"><option value="">-- Formation --</option>{formations.map(f=><option key={f.id} value={f.id}>{f.name}</option>)}</select><select value={newScheduleData.formateurId} onChange={e=>setNewScheduleData({...newScheduleData, formateurId: e.target.value})} className="input-style"><option value="">-- Formateur --</option>{formateurs.map(f=><option key={f.id} value={f.id}>{f.name}</option>)}</select><select value={newScheduleData.dayOfWeek} onChange={e=>setNewScheduleData({...newScheduleData, dayOfWeek: e.target.value as any})} className="input-style"><option>Lundi</option><option>Mardi</option><option>Mercredi</option><option>Jeudi</option><option>Vendredi</option><option>Samedi</option><option>Dimanche</option></select><input type="time" value={newScheduleData.startTime} onChange={e=>setNewScheduleData({...newScheduleData, startTime: e.target.value})} className="input-style"/><input type="time" value={newScheduleData.endTime} onChange={e=>setNewScheduleData({...newScheduleData, endTime: e.target.value})} className="input-style"/><div className="flex justify-end"><button type="submit" className="btn-primary bg-academie-blue">Enregistrer</button></div></form></Modal>
            <Modal isOpen={isPaymentModalOpen} onClose={() => setIsPaymentModalOpen(false)} title={`Ajouter un paiement pour ${selectedStudent?.name}`}><form onSubmit={e => {e.preventDefault(); handlePaymentSubmit()}} className="space-y-4"><input type="number" placeholder="Montant" value={newPaymentData.amount} onChange={e=>setNewPaymentData({...newPaymentData, amount: e.target.value})} className="input-style" required/><select value={newPaymentData.objet} onChange={e=>setNewPaymentData({...newPaymentData, objet: e.target.value as any})} className="input-style"><option>Inscription</option><option>Tranche 1</option><option>Tranche 2</option><option>Solde</option></select><div className="flex justify-end"><button type="submit" className="btn-primary bg-green-600">Enregistrer</button></div></form></Modal>
