                 ...
                 <form onSubmit={e => {e.preventDefault(); handleOrderSubmit()}} className="space-y-4">
                    ...
                    <div className="border p-2 rounded-md">
                        <div className="grid grid-cols-10 gap-2 items-end">
                            <div className="col-span-4"><label className="text-sm">Article</label><select value={itemToAdd.articleId} onChange={e=>setItemToAdd({...itemToAdd, articleId: e.target.value})} className="input-style">{articles.map(a=><option key={a.id} value={a.id}>{a.name}</option>)}</select></div>
                            <div className="col-span-2"><label className="text-sm">Qté</label><input type="number" min="1" value={itemToAdd.quantity} onChange={e=>setItemToAdd({...itemToAdd, quantity: +e.target.value})} className="input-style"/></div>
                            <div className="col-span-2"><label className="text-sm">Prix Achat/U</label><input type="number" value={itemToAdd.purchasePrice} onChange={e=>setItemToAdd({...itemToAdd, purchasePrice: +e.target.value})} className="input-style"/></div>
