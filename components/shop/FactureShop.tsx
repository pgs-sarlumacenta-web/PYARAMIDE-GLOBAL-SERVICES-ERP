...
const FactureShop: React.FC<FactureShopProps> = ({ order, articles, client, companyProfile, billingSettings }) => {
    ...
    <div className="bg-white text-black font-sans w-full max-w-2xl mx-auto p-8 border border-gray-300 shadow-lg">
      <header className="flex justify-between items-start pb-6 border-b border-gray-300">
        ...
        <div className="text-right">
          ...
          <p className="text-gray-500">N°: {order.factureRef}</p>
            ...
            {order.items.map((item, index) => {
              ...
              return (
                <tr key={index} className="border-b">
                  ...
                  <td className="p-3 align-top text-right text-red-600">{item.discount ? `- ${item.discount.toLocaleString()} GNF` : '-'}</td>
      ...
      <section className="flex justify-end mt-8">
        <div className="w-full max-w-sm text-right">
          ...
          {totalDiscount > 0 && <div className="flex justify-between py-1 text-red-600"><span>Remise Totale:</span><span>- {totalDiscount.toLocaleString()} GNF</span></div>}
          <div className="flex justify-between py-1 font-bold border-t"><span>Total:</span><span>{order.totalAmount.toLocaleString()} GNF</span></div>
          <div className="flex justify-between py-1 text-green-600"><span>Montant Payé:</span><span>- {order.amountPaid.toLocaleString()} GNF</span></div>
          <div className="flex justify-between py-3 border-t-2 border-black mt-2"><span className="font-bold text-lg">Solde Dû:</span><span className="font-bold text-lg">{balance.toLocaleString()} GNF</span></div>
        </div>
      </section>
      
      <footer className="text-center mt-12 pt-6 border-t border-gray-300">
        <p className="text-xs text-gray-500 whitespace-pre-wrap">{billingSettings.defaultFooter}</p>
        <p className="text-gray-600 font-semibold mt-2">Merci pour votre achat !</p>
      </footer>
    </div>
  );
};

export default FactureShop;